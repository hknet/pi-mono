...
describe("openai-codex streaming", () => {
	...
	it("surfaces a friendly message for usage_limit_reached error events", async () => {
		const tempDir = mkdtempSync(join(tmpdir(), "pi-codex-stream-"));
		process.env.PI_CODING_AGENT_DIR = tempDir;
		const token = mockToken();
		const encoder = new TextEncoder();
		...
		const errorEvent = {
			...
			error: {
				...
				resets_at: resetsAt,
				eligible_promo: null,
				resets_in_seconds: 30 * 60,
			},
			status_code: 429,
			headers: { "X-Codex-Plan-Type": "plus" },
		};
		...
		const result = await Promise.race([
			streamOpenAICodexResponses(model, context, { apiKey: token, transport: "sse" }).result(),
			new Promise<never>((_, reject) => {
				setTimeout(() => reject(new Error("Timed out waiting for error SSE stream")), 1000);
			}),
		]);

		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toMatch(
			/^You have hit your ChatGPT usage limit \(plus plan\)\. Try again in ~\d+ min\.$/,
		);
		expect(result.errorMessage).not.toContain("Codex error:");
